...
def prop_FC(csp, newVar=None):
    ...
    if newVar:
        cons_iteration = csp.get_cons_with_var(newVar)
    else:
        cons_iteration = csp.get_all_cons()
    for cons in cons_iteration:
        if cons.get_n_unasgn() == 1:
            unasgn_var = cons.get_unasgn_vars()[0]
            result = singleFC(cons, unasgn_var, pruned)
...
def singleFC(con, var, pruned):
    ...
    for d in curdom:
        ...
        if not con.has_support(var, d):
            if (var, d) not in pruned:
                var.prune_value(d)
                pruned_FC.append((var, d))
        
    if var.cur_domain() == []:
        flag = False
    return flag, pruned_FC
...
def prop_GAC(csp, newVar=None):
    ...
    if newVar:
        ...
        cons_iteration = csp.get_all_cons()    
    while len(cons_iteration) > 0:
        con  = cons_iteration.pop(0)
                ...
                if not con.has_support(var, d):
                    ...
                    if var.cur_domain() == []:
                        ...
                        for con_prime in csp.get_cons_with_var(var):
                            if con_prime not in cons_iteration:
                                cons_iteration.append(con_prime)
    return True, pruned
