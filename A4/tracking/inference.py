    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            ghostPosition = self.setGhostPosition(gameState, oldPosition)
            newPosDist = self.getPositionDistribution(ghostPosition)
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob * self.beliefs[oldPosition]
